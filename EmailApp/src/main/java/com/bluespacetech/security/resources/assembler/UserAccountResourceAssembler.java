...
/**
 * The Class UserAccountResourceAssembler.
 *
 * @author pradeep created date 27-Jan-2015
 */
public class UserAccountResourceAssembler extends ResourceAssemblerSupport<UserAccount, UserAccountResource>
{

    /**
     * Instantiates a new user account resource assembler.
     */
    ...
    public UserAccountResource toResource(final UserAccount userAccount)
    {
        ...
        final Link link = linkTo(UserAccountController.class).slash(userAccount.getId()).withSelfRel();
        userAccountResource.add(link.withSelfRel());
        return userAccountResource;
    }

    /**
     * To complete resource.
     *
     * @param userAccount the user account
     * @param userGroupsMap the user groups map
     * @return the user account resource
     */
    public UserAccountResource toCompleteResource(final UserAccount userAccount,
            final Map<Long, UserGroup> userGroupsMap)
    {
        ...
        if (userAccount.getUserAccountUserGroups() != null)
        {
            for (final UserAccountUserGroup userAccountUserGroup : userAccount.getUserAccountUserGroups())
            {
                ...
                final UserGroupResource userGroupResource = userAccountResourceAssembler
                        .toResource(userGroupsMap.get(userAccountUserGroup.getUserGroupId()));
    ...
    /**
     * Copy all the properties from source Financial year to destination Financial year.
     *
     * @param sourceUserAccount source Financial year.
     * @param destinationUserAccount destination Financial year.
     */
    public static void copyUserAccountInto(final UserAccount sourceUserAccount,
            final UserAccount destinationUserAccount)
    {
        ...
        if (destinationUserAccount.getUserAccountUserGroups() != null)
        {
            ...
            for (final UserAccountUserGroup newUserAccountUserGroup : sourceUserAccount.getUserAccountUserGroups())
            {
                ...
                {
                    userAccountUserRoleToPersist
                            .add(existingUserAccountUserGroups.get(newUserAccountUserGroup.getResourceObjectId()));
                }
                else
                {
     ...
     * @return the user account from resource
     */
    public static UserAccount getUserAccountFromResource(final UserAccountResource userAccountResource)
    {
        ...
        {
            for (final UserAccountUserGroupResource userAccountUserGroupResource : userAccountResource
